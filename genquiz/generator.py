...
class Quiz:
    ...
    def __add(self, item):
        ...
        if isinstance(item, ShowFigure):
            self.__add_shw_figure(item)
    ...
    def __save(self, question_path, solution_path):
        # print(self.student_id.replace('-', ''))
        self.doc.save(os.path.join(question_path, self.student_id.replace('-', '') + '.docx'))
        pickle.dump(self.solutions, open(os.path.join(solution_path, self.student_id.replace('-', '') + '.pk'), 'wb'))
...
class Student:
    def __init__(self, path):
        ...
        # self.name = df.iloc[:, 1].values+' '+df.iloc[:, 2].values
        self.name = df.iloc[:, 1].values
